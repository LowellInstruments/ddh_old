...
def get_firmware_battery(path):
    path = path.replace(".bin", ".csv")
    data_info = pd.read_csv(path, nrows=8)
    if 'attery' in data_info.iloc[5][0]:
        battery=data_info.iloc[5][1]
        f_version=data_info.iloc[1][1].split('-')[1]
        print('battery is'+ battery)
        print('f_version is '+ f_version)
      	#sn=data_info.iloc[0][1]
    else:
        battery=99
        f_version=99
    return battery,f_version
    ...
    with open(ofn, "w") as fw:
        with open(path, "r") as fr:
            ...
            for i in ll:
                ...
                if float(_dbar) < 0:
                    continue
                # line_out: DATA,2023-03-10T11:11:23,lat,lon,5.2,1.4
                lo = "{},{},{},{},{:.2f},{:.1f}\n"
                lo = lo.format(
                    "DATA", line_date_time.replace('T',' ').split('.')[0], lat, lon, float(_temp), float(_dbar)
                )
                fw.write(lo)
...
def file_out_hl_process_xc_85(path) -> EmoltMsgShortHaul:
    ...
    std_t_df_85 = _t.std()
    #datetime.strptime(df_85['datet(GMT)'].iloc[-1], '%Y-%m-%d %H:%M:%S')
    time_delta = (datetime.strptime(df_85['datet(GMT)'].iloc[-1], '%Y-%m-%d %H:%M:%S') - datetime.strptime(df_85['datet(GMT)'].iloc[0], '%Y-%m-%d %H:%M:%S'))
    total_seconds = time_delta.total_seconds()
    minutes = total_seconds//60
    m_haul_duration=minutes
    ...
    #print("\tm_haul_duration  = {:.2f}".format(m_haul_duration))
