...
PERIOD_GPS_TELL_NUM_SATS_SECS = 60
...
def gps_check_for_errors(g) -> int:
    ...
    # don't log GPS or notify error too often
...
def gps_power_cycle_if_so(forced=False):
    ...
    try:
        ...
        # output stream seems OK, but be really sure
        if b"+QGPS: 1" in ans:
            bb = _sp.readall()
            if (b'GPGSV' in bb or b'GPGSA' in bb or
                    b'GPRMC' in bb or b',,,,' in bb):
                did_configure_ok = True
            else:
                lg.a("error: power-cycle needed, no GPS OUT stream")
                _gps_power_cycle_via_pin()

        # output stream answers but not enabled, enable it
        elif b"+QGPS: 0" in ans:
            _sp.readall()
            _sp.write(b"AT+QGPS=1\r")
            ans = _sp.readall()
            if b"OK" in ans:
                lg.a("OK: GPS shield check from 0 to 1")
                did_configure_ok = True
            else:
                lg.a(f"error: GPS shield check cannot set to 1 -> {ans}")
        ...
        lg.a(f"error: failed gps_power_cycle -> {ex}")
        _gps_power_cycle_via_pin()
...
def gps_configure_shield():
    return gps_power_cycle_if_so(forced=True)
...
def _gps_measure():
    ...
    # open serial port
    if _g_bu353s4_port:
        _sp = serial.Serial(_g_bu353s4_port, 4800, timeout=0.2)
    else:
        _sp = serial.Serial(g_quectel_port_usb_gps, baudrate=115200, timeout=0.2,
                           rtscts=True, dsrdtr=True)

    global _g_ts_cached_gps_valid_for
    global _g_cached_gps

    # read GPS data from port
    _sp.readall()
    _sp.reset_input_buffer()
    ...
    # keep only the last bytes written
    b = b[-100:]
    ...
    # failed, and first time ever, mostly happens when booting DDH
