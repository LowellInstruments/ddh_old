...
from utils.ddh_config import dds_get_cfg_flag_rbl_en, dds_get_cfg_box_sn
...
def rbl_decode(b: bytes):
    ...
    #     s = build_rbl_build_emolt_msg_as_str()
    #     b = rbl_hex_str_to_hex_bytes()
    # then 'b' travels up to Rockblocks+ webservice, which does the
    # inverse operation, 'b' to 's' and sends 's' to our DDN webservice
    # -------------------------------------------------------------------
    ...
    # we at Lowell NEVER use this function, just showcased for NOAA
...
def rbl_serve():
    ...
    # find RBL files, generated by BLE downloads
...
if __name__ == '__main__':
    ti = int(time.time())
    ts = hex(ti)[2:]
    _b = '00000000000000000000' + ts
    _b += '000000000000000000000000000000000000000000000000000000'
    _rbl_send(_b.encode(), 'bin')
